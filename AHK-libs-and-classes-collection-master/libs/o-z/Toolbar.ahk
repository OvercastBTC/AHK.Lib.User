...
Toolbar_Clear(hCtrl)
{
	...
	Toolbar_MEM_free( Toolbar( hCtrl "aBTN", "" ) )
...
Toolbar_DeleteButton(hCtrl, Pos=1)
{
	...
	if InStr(Pos, "*") {
		...
		if (Pos > cnta)
			return FALSE
		if (Pos < cnta)
			Toolbar_MEM_move( aBTN + 20*(Pos-1) +4, aBTN + 20*Pos +4, aBTN + 20*Pos +4)
...
; Parameters:
; 		Btns		- The button definition list.
; 						* Each button to be added is specified on separate line using button definition string. Empty lines will be skipped.
...
Toolbar_Insert(hCtrl, Btns, Pos=""){
	...
	Toolbar_MEM_free(cBTN)

    ; Note: for some reason, you need to call this 2 times for proper results in some scenarios .... !?
	SendMessage,0x421,,,,ahk_id %hCtrl%	;autosize
	SendMessage,0x421,,,,ahk_id %hCtrl%	;autosize
}
...
Toolbar_ToggleStyle(hCtrl, Style="LIST"){
	...
	; bug: This didn't work...
	; bug: SendMessage, TB_GETSTYLE, ,,, ahk_id %hCtrl%
	; bug: style := (ErrorLevel & style) ? ErrorLevel & !style : ErrorLevel | style
	; bug: SendMessage, TB_SETSTYLE, 0, style,, ahk_id %hCtrl%
...
; --------------------------------------------------------------------------------
; Method: Parse button definition list and compile it the into binary array. Add strings to pull. Return number of current buttons.
; Variables:
...
Toolbar_compileButtons(hCtrl, Btns, ByRef cBTN)
{
	...
	if (aBTN = "")
		aBTN := Toolbar_MEM_alloc( 50 * 20 + 4),  Toolbar(hCtrl "aBTN", aBTN)	 ;if space for array of * buttons isn't reserved and there are definitions of * buttons reserve it for 50 buttons + some more so i can keep some data there...

	StringReplace, _, Btns, `n, , UseErrorLevel
	btnNo := ErrorLevel + 1
	cBTN := Toolbar_MEM_alloc( btnNo * 20 )
	...
	;  calculate icon
		; if user didn't specify icon, use button number as icon index (don't count separators)
		if a2 is not Integer					
			a2 := cnt+cnta+1-sep
		; calculate offset o of this structure in array of TBBUTON structures.
		o += 20 * (a ? cnta : cnt)				
		...
		} else sIdx := -1,  a2 := (StrLen(A_LoopField)-1)*10 + 1
		; if separator, lentgth of the "-" string determines width of the separation. Each - adds 10 pixels.
	...
	; return number of buttons in the array
	return cnt									
}
...
;   =================== CUSTOMIZATION NOTIFICATIONS ===========================
	...
	if (code = TBN_GETBUTTONINFOA)   {
		...
		Toolbar_MEM_copy( TBB, o, 20) ;copy the compiled item into notification struct
		return 1
	}
...
Toolbar_onEndAdjust(hCtrl, cBTN, cnt)
{
	...
	buf := Toolbar_MEM_alloc( size * 20 + 4)
	...
	loop, %cnt%
	{
		...
		if NumGet(o+0, 9, "Char") = BTNS_SEP				;skip separators
			continue
		Toolbar_MEM_copy( buf + cnta2++*20 +4, o, 20) ;copy the button struct into new array
	}
	Toolbar_MEM_free(cBTN)
