...
Hzn := 'ahk_exe ApplicationFrameHost.exe'
...
fCtl := WinGetList('A')
myVisitControlsList := []
myVisitWinGetControls := WinGetControls('A')
myVCtrl := ''
myVCtrl_list := ''
myVlist := ''
for each, myVCtrl in myVisitWinGetControls {
	myVisitControlsList.Push(myVCtrl)
	; myVCtrl_list .= 'myVCtrl: ' . myVCtrl_list . '`n'
}
for each, myVlist in myVisitControlsList {
	OutputDebug('myVlist: ' . myVlist '`n')
}
Ctrl := ''
myVwin := ''
arrmyV := ''
arrVar := ''
for each, Ctrl in fCtl {
	tCtl := WinGetTitle(Ctrl)
	nCtl := WinGetClass(tCtl)
	myVpname := WinGetProcessName(tCtl)
	myVlist := WinGetList(myVpname)
	for each, arrVar in myVlist {
		arrmyV .= 'list: ' . arrVar . '`n'
		if (arrmyV = '') {
			ttl := WinGetTitle('A')
		}
		; OutputDebug('arrmyV' . arrmyV . '`n')
	}
	myVwin .= 'tCtl(title)[' tCtl . ']' . '`n' 
	. 'nCtl(class)[' . nCtl . ']' . '`n'
	. 'myVpname[' . myVpname . ']' . '`n'
	. 'arrmyV[' . arrmyV . ']' . '`n'
	. '`n'
}
OutputDebug('myVwin:`n' myVwin . '`n')
try {
	...
	OutputDebug('It`'s MyVisit: ' . process_name . ' (' . hHzn . ')`n' 
				. 'process_hwnd_dll: ' . process_hwnd_dll . '`n' 
				. 'name_process_ID_dll: ' . name_process_ID_dll 
				. ' process_ID_dll: ' . process_ID_dll . '`n'
			)
	; initial_list_ClassNN := WinGetControls('A')

	OutputDebug('nCtl: ' . nCtl . ' (' . fCtl . ')' . '`n')
	initial_list_ClassNN := WinGetControls('A')
	classNN := ''
	for each, classNN in initial_list_ClassNN {
		; try {
		initial_list_ClassNN_hwnd := ControlGetHwnd(initial_list_ClassNN, 'A')
		OutputDebug('initial_list_ClassNN: ' . classNN . 'initial_list_ClassNN_hwnd' . initial_list_ClassNN_hwnd . '`n')
...
HandleWinEvent(hWinEventHook, event, ghwnd, idObject, idChild, idEventThread, dwmsEventTime) {
	...
	; static pneedle := 'i).*ApplicationFrameHost.exe.*'
	hznCtrls := ''
	matchTbClassNN := ''
	matchTbHwnd := ''
	matchTb := ''
	; needle := 'im)(\w+\sbar\w+)'
	static hwnd := WinExist('ahk_exe ApplicationFrameHost.exe')
	...
	while !(process_name ~= needle){
		break
	}
	if (process_name ~= needle) && (WinGetProcessName(WinActive('A')) ~= needle){
		try {
		...
		try {
			for each, matchTb in hznmsvbTbHwnd {
				if !(hznmsvbTbHwnd.Has(matchTb)){
					try {
						OutputDebug('event: ' . event . 'matchTb: ' . matchTb . '`n')
...
A_Process_GetInfo(event, hwnd,*) {
	...
	if ((event = 1) || (event = 3) || (event = 4) || (event = 32772) || (event = 14) || (event = 12)) {
		...
		A_Process := name
		p_list := dll_list_processes()
		val_p_list := ''
		enum_val_p_list := ''
		for each, val_p_list in arr_p_list {
			enum_val_p_list .= val_p_list
		}
		global stop := A_TickCount
		elapse := (stop - start)
		HznToolTip()
	}
	; --------------------------------------------------------------------------------
	HznToolTip() {
		...
		ToolTip('A_Process: ' . A_Process . ' '
			...
			. 'time elapsed (msec): (' . elapse . ')`n'
			; . p_list . '`n' 
			; . enum_val_p_list . '`n'
			, 0, 0
		)
		FileAppend('Event [' . (FormatTime(A_Now,'yyyy.MM.dd HH:mm:ss')) . ']' . '`n`n' . 'A_Process: ' . A_Process . ' '
...
dll_list_processes(){
	...
	p_list := StrReplace(l, "`n", d), c " Processes", 0
	return p_list
}
