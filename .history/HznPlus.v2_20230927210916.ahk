...
; Hook_TB_CREATED := DllCall('SetWinEventHook', 'UInt', 0x8, 'UInt', 0x8, 'UInt', 0, 'UInt', msvb_lib_toolbar_created, 'UInt', 0, 'UInt', 0, 'UInt', 0)

; Pass control to EnumWindows(), which calls the callback repeatedly:
DllCall("EnumWindows", "Ptr", EnumAddress, "Ptr", 0)

; CallBack_TB_CREATED(hWinEventHook, Event, hWnd, idObject, idChild, dwEventThread, dwmsEventTime) {
...
{
	global Output
	WinGetCtrls := WinGetControls('A')
	Loop parse WinGetCtrls
		Instance := RegExMatch(WinGetCtrls, 'i)msvb_lib_toolbar\d')
	
	win_title := WinGetTitle('msvb_lib_toolbar*')
