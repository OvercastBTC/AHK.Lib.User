...
class update_script {
	...
	static updateLib() {
		...
		bDrive_Connected() {
			...
			static persLib := this.persRepo '\Lib\TestingStuff'
			static mainLib := this.persLib '\TestingStuff'
			If !DirExist(this.mainLib) {
				DirCreate(this.mainLib)
			}
			If !DirExist(this.persRepo) {
				DirCreate(this.persRepo)
			}
			If !DirExist(this.persLib) {
				DirCreate(this.persLib)
			}
