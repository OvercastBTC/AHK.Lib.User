...
class update_script {
	...
	static updateLib() {
		...
		bDrive_Connected() {
			...
			static mainLib := persLib '\mainLib'
			If !DirExist(mainLib) {
				DirCreate(mainLib)
			}
			...
			; 	mainLib '(not intended for editing).'
			; 	'`n'
			...
			;? Open Powershell and expand the zip file to the selected folder. "-Force" will overwrite files with name conflicts. Powershell will close after successful operation.
			RunWait("PowerShell.exe -WindowStyle hidden -Command Expand-Archive -LiteralPath " this.Input_Lib_File " -DestinationPath " this.Output_Lib_File " -Force") 
			...
			ClickedInstall(*) {
				HrznGui.Destroy()
				;? Open Powershell and expand the zip file to the selected folder. "-Force" will overwrite files with name conflicts. Powershell will close after successful operation.
				RunWait("Powershell.exe -WindowStyle hidden -Command Expand-Archive -LiteralPath " this.Input_HznPlus_File " -DestinationPath " PS_Output_Folder " -Force") 
			}
