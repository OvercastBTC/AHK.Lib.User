EnumToolbarButtons(ctrlhwnd, is_apply_scale:=false) {
	...
	pid_target := WinGetPID("ahk_id " ctrlhwnd)
	hpRemote := DllCall("OpenProcess", "uint", 0x18, "int", false, "uint", pid_target) 
    ; hpRemote: Remote process handle
	if(!hpRemote) {
		ToolTip("Autohotkey: Cannot OpenProcess(pid=" . pid_target . ")")
		return
	}
	Buffer := DllCall("VirtualAllocEx", "uint", hpRemote, "Ptr", 0, "uint", 0x1000, "uint", 0x1000, "uint", 0x4)
	x1 := ""
	x2 := ""
	y1 := ""
	WM_USER					:= 0x400
	...
	bCount := SendMessage(TB_BUTTONCOUNT, 0, 0, , "ahk_id " ctrlhwnd)
...
;		SendMessage, %TB_GETSTATE%, %idButton%, 0, , ahk_id %ctrlhwnd% ; hope that 4KB is enough ; just a test
		...
		if(btntextchars>0){
			btntextbytes := 1 ? btntextchars*2 : btntextchars
