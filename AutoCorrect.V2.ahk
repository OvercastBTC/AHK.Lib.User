...
#Include AutoCorrect-FM-Brand_Assurance.V2.ahk
;@Ahk2Exe-IgnoreBegin
; #Include \AutoCorrect-Personal-Additions.V2.ahk
;@Ahk2Exe-IgnoreEnd
;------------------------------------------------------------------------------
...
; CHANGELOG:
;
; 3 Nov 2023: Updated the validate and append funtions to look at multiple files
;             and append to the "personal" file, so this script could be moved
;             into the library folder.
;
;
; 21 August 2023: Several modifications to incorporate improvements made by kunkel321 that were
; 				  posted in the AHK forums. https://www.autohotkey.com/boards/viewtopic.php?f=83&t=120220&p=533498&hilit=autocorrect#p533498
;
;
; 17 August 2023: Initial conversion from V1 to V2. Mostly a rework of the Win+H hotkey
;                 and some other minor cleanup.
; 
; Based on the original AutoHotkey v1 AutoCorrect script released by Jim Biancolo (http://www.biancolo.com)
; You can view the original v1 script at https://www.autohotkey.com/download/AutoCorrect.ahk
; 
; CONTENTS
; 
;   Win+H code
...
;
; This is commented out as it causes more misfires than fixes.
;------------------------------------------------------------------------------
/* 
fix_consecutive_caps()
fix_consecutive_caps() {
  If WinActive("ahk_exe WINWORD.EXE") Or WinActive("ahk_exe EXCEL.EXE") Or WinActive("ahk_exe OUTLOOK.EXE") Or WinActive("ahk_exe ONENOTE.EXE") Or WinActive("ahk_exe POWERPNT.EXE") Or WinActive("ahk_exe MSACCESS.EXE") Or WinActive("ahk_exe MSPUB.EXE"){   
    return
  }
  Else{
    ; Hotstring only works if CapsLock is off.
        ...
        fix(char1, char2, *) {
            ...
            OnEnd(ih) {
                ...
                || (char3 = A_Space && char1 char2 ~= "OF|TO|IN|AT|WE|HE|BY|ON|BE|NO") ; Fix two letter words.
...
{ 
	...
	MakeGui()
	MakeGui(){
		hh := Gui('', "Hotstring Helper -- Muli-Line")
		...
		global DefHotStr := hh.AddEdit('vDefHotStr x+28 w' . wFactor + 250, '')
		...
		global RepStr := hh.AddEdit('vRepStr +Wrap y+1 xs h' . hFactor + 100 . ' w' . wFactor + 320, '')
		global ComLbl := hh.AddText('xm y' . hFactor + 182, 'Enter Comment')
		hh.SetFont("s11 cGreen")
		global ComStr := hh.AddEdit('vComStr xs y' . hFactor + 200 . ' w' . wFactor + 315)
		; ---- Buttons
		global (ButApp := hh.AddButton('xm y' . hFactor + 234, '&Append')).OnEvent("Click", hhButtonAppend)
		global (ButVal := hh.AddButton('+notab x+5 y' . hFactor + 234, '&Validate')).OnEvent("Click", hhButtonValidate)
		global (ButSpell := hh.AddButton('+notab x+5 y' . hFactor + 234, '&Spell')).OnEvent("Click", hhButtonSpell)
		global (ButCancel := hh.AddButton('+notab x+5 y' . hFactor + 234, '&Cancel')).OnEvent("Click", hhButtonCancel)
	; }
	MyDefaultOpts := ""
	DefaultHotStr := ""
	Global myPrefix := ""
	...
	SubTogSize(hFactor, wFactor) {
		global DefHotStr, RepStr, ComLbl, ComStr, ButApp, ButVal, ButSpell, ButCancel
	...
	HotIf() WinActive("Hotstring Helper -- Muli-Line", ) ; Allows window-specific hotkeys.
	HotKey('$Enter', hk_Enter())
	; {
	...
	HotKey('Esc', () => hh.Hide() => A_Clipboard := ClipboardOld)
	...
	ValidationFunction(tMyDefaultOpts, tDefHotStr, tRepStr) { 
		...
		AutoCorrectBAFile := A_ScriptDir "\Lib\AutoCorrect-FM-Brand_Assurance.V2.ahk"
		AutoCorrectPersonalFile := A_ScriptDir "\AutoCorrect-Personal-Additions.V2.ahk"
		Source_1 := Fileread(A_ScriptName)
		Source_2 := Fileread(AutoCorrectBAFile)
		Source_3 := Fileread(AutoCorrectPersonalFile)
		ThisFile := Source_1 Source_2 Source_3  ; Save these contents to variable 'ThisFile'.
		...
		If(WithNeedlesRemoved = ""){ ; If they were all removed...
			...
			; Just a block text assignement to var
			OptTips := " 
			(
		...
		If (tDefHotStr = "") || (tDefHotStr = myPrefix) || (tDefHotStr = mySuffix) || InStr(tDefHotStr, ":")
			...
			If instr(A_LoopField, ":" . tDefHotStr . "::") {
				; If line contains tDefHotStr...
	...
	Else {
		...
		AutoCorrectPersonalFile := A_ScriptDir "\AutoCorrect-Personal-Additions.V2.ahk"
		FileAppend("`n" WholeStr, AutoCorrectPersonalFile) ; 'n makes sure it goes on a new line.
		Reload() ; relaod the script so the new hotstring will be ready for use.
	}

	; Called is "Spell" because "Spell Check" is too long.
	hhButtonSpell(*) {
		tRepStr := hh['RepStr'].text
		If (tRepStr = ""){
...
; The below word ending additions are from https://www.autohotkey.com/boards/viewtopic.php?f=83&t=120220&p=533498&hilit=autocorrect#p533498

:*:dicht::dichot ; Fixes 18 words. MisspeLLs "Mulloidichthys" a genus of MuLLidae (goatfishes or red mullets).
:*?:bouy::buoy ; Fixes 13 words. Misspells Bouyie (a branch of Tai Language).
:*?:bred::broad ; Fixes 55 words. Misspells brodiaea (a type of plant)
...
:*?:naton::nation ; Fixes 451 words but misspells Akhenaton (EarLy ruLer of Egypt who rejected aid gods and replaced with sun worship, died 1358 BC).
:*?:nnst::nst ; Fixes 729 words} misspells Dennstaedtia (fern)j Hoffmannsthalj (poet)
:C*?:sigin::sign ; Fixes 243 words. Misspells SIGINT "Info from electronics telemetry intel."
...
:C*?:comt::cont ; fixes 587 words. Misspells vicomte (French nobleman); :C: so not misspell Comte (founder of Positivism and type of cheese)
...
::diamons::diamonds
::diarhea::diarrhoea
; ::dichtomy::dichotomy ; Commented out as this is covered by word endings additions
::didnot::did not
::didint::didn't
::didnt::didn't
...
::busineses::businesses ;business,
::carmel::caramel ;, carmel-by-the-sea
::clera::clear ;, sclera
::fontrier::frontier ;fontier, 
::gaurd::guard ;, gourd
::gogin::going ', Gauguin
::haev::have ;, heave
::herad::heard ;, Hera
...
::profesion::profession ;profusion, 
::progrom::program ; pogrom, program
::restraunt::restaurant ;restraint, restaurant
::spects::aspects ;, expects
::strat::start ;, strata
::stroy::story ;, destroy
::tiem::time ;, Tim
::tiome::time ;time, tome
::tourch::torch ;touch
::travelling::traveling   ; commonwealth vs US
::troups::troops ; troupes, troops
::turnk::trunk ; turnkey, trunk
::unmanouverable::unmaneuverable ;, unmanoeuvrable
::villin::villain ; villi, villein
::vistors::visitors ;, vistas
::wholy::wholly ;, holy
::woulf::would ;, wolf
::rouge::rogue ;rouge is be a legitimate word, but I would not use it (at least at work)
